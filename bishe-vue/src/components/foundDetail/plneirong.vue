...
<el-card class="pbox-card">
  <div class="ptext pitem">
    <div class="fxbiaoti" >{{communication.communication_name}}</div>
    <div style="font-size:5px;float:right;color:gray;" @click="backtoup2()">查看全部回答 ></div>
  </div>
</el-card>

  <el-card class="box-card"  >   <!--第yi个卡片 -->

      <div slot="header" class="clearfix">
    <span class="nryonghu" @click="tozhuye()">
      <div class="nrkuang">
      <img :src="content.user_img" width="30" height="30" />
      </div>
      <span class="nrmingzi">{{content.user_name}}</span>
    </span>
...
<mt-button type="primary" size="small" @click="topush()" class="wfabu">发布</mt-button>
  ...
  <el-card class="box-card"  v-for="(huifu,index) in huifus" :key="index">   <!--第yi个卡片 -->
        <div class="text item">
         
          <div class="htyonghu">
            <div class="htkuang">
            <img  :src="huifu.user_img" width="15" height="15">
            </div>
            <div class="htname">{{huifu.user_name}}</div>
          </div>
    ...
    towrite(){
      if(this.pshow){  
          ...
          topush(){
             ...
             var comment_id = _this.$route.params.comment_id
             ...
             var time = _this.currentDate
                _this.$http.post("http://localhost:3000/comment_list/addcontent",{communication_id:communication_id,comment_id:comment_id,user_name:user_name,user_img:user_img,comment:comment,time:time},{emulateJSON: true})
            .then(
              (response)=>{
                    ...
                    if(response.data.ok==1){
                     _this.$http.post("http://localhost:3000/comment_list/findOne",{communication_id:communication_id,comment_id:comment_id},{emulateJSON: true})
            .then(
              (response)=>{
                      //  console.log(response.data)
                     ...
                     for(var i = 0; i < _this.comments.length; i++){
                         if(comment_id == _this.comments[i]._id){
                             _this.content = _this.comments[i]
                         }
                     }
                     
                           _this.huifus =  _this.comments[0].contents
                           _this.pshow = true;  
                           console.log(_this.comments[0].contents)
                             console.log(_this.huifus)
              },
              (error)=>{
